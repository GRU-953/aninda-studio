...
| App Store localisations *(added 26 Aug 2026)* | The metadata language list includes **Bangla**, added **30 March 2026** in a batch of eleven. Apple names it "Bangla", not "Bengali". | Apple accepts store metadata in that language and uses the same name for it that this studio does. Both store packages carried English and Bangla metadata on the strength of this row until **27 August 2026**, when the Bangla left the system; they ship English now. The row is kept because what Apple publishes has not changed, and because a listing that could be bilingual and is not is a choice worth being able to see. |
...
28 testable criteria, each naming the test that decides it. **The verdicts were run against the finished kit on 19 August 2026**, one agent per batch, each executing the named test rather than forming an opinion. The evidence for every row — the commands and what they returned — is in `_data/benchmark-verdicts.json`. Criteria 12, 20 were re-scored on 2026-08-28, against the same tests, after the work they named was done. Criteria 1, 6, 7, 23 were re-scored on 26 August 2026, against the same tests, after the work they named was done.
...
| 23 | Any dependency on Material Expressive records the **library version and its channel** (stable 1.4.0 versus 1.5.0-alpha) | Read the dependency manifest: version and channel must both be present | **Met** — Kotlin now exists and the version and channel are both recorded. No manifest declares a Material dependency: the emitted Kotlin is framework-free and imports no androidx. |
