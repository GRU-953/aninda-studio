...
28 testable criteria, each naming the test that decides it. **The verdicts were run against the finished kit on 19 August 2026**, one agent per batch, each executing the named test rather than forming an opinion. The evidence for every row — the commands and what they returned — is in `_data/benchmark-verdicts.json`. Criteria 12, 20, 23, 25 were re-scored on 2026-08-28, against the same tests, after the work they named was done. Criteria 1, 6, 7 were re-scored on 26 August 2026, against the same tests, after the work they named was done.

Result: 24 met · 4 part met.

This table is generated by `scripts/benchmark.py` from that data file. Editing it here is undone by the next build; change the verdict in the data and regenerate. Until this pass, all 28 cells read "—" — the criteria had never been run.

| # | Criterion | The test that decides it | Verdict |
|---|---|---|---|
| 1 | Icon masters are exported **unmasked**, at 1024×1024 px for iOS, iPadOS and macOS, and 1088×1088 px for watchOS | Open each exported file: check pixel dimensions, and check the four corners are opaque artwork rather than transparent rounded-off area | **Met** — Both masters now exist square and unmasked at the sizes Apple publishes, and their corners are opaque artwork. |
| 2 | The mark survives a **circular** mask, not only a rounded square | Apply a circular mask to the master and confirm no essential part of the mark is clipped | **Met** — The furthest mark pixel sits at 86.5% of the inscribed circle radius and zero mark pixels fall outside the circle, at both stroke weights. |
| 3 | **No corner-radius figure anywhere in the kit is attributed to Apple** | Search all tokens and prose for "22.37", "22.46", "squircle" and "corner radius"; every surviving figure must be labelled as this kit's own | **Met** — Every surviving hit is a negation or labelled the kit's own; the sole 22.46 mention records a removed defect, and radius tokens carry no Apple attribution. |
| 4 | Icon source is layered as background plus foreground, within a maximum of **four groups**, with text converted to outlines | Open the SVG: count top-level groups; confirm no `<text>` element remains | **Met** — Each icon parses to one top-level group over a background rect, inside the four-group limit, with zero text or tspan elements present. |
| 5 | Icon source contains **no baked blur, shadow, specular highlight, opacity or background gradient** | Search the SVG for `filter`, `feGaussianBlur`, `opacity` and gradient definitions; each hit must be justified or removed | **Met** — Zero hits for filter, feGaussianBlur, opacity, gradients, defs or clip-path across all shipped icon SVGs; the one mask substring is the word unmasked in a title. |
| 6 | A **Mono** (single-colour) variant exists and is legible at both full size and the smallest specified size | Render the Mono layer at both sizes and confirm the mark reads | **Met** — A Mono appearance layer is designated and rendered, and the mark reads at full size and at the documented floor. |
| 7 | Icon masters are authored in **sRGB**; any Display P3 variant is additional, and no P3 asset is offered for visionOS | Read the embedded colour profile of every exported asset | **Met** — Every raster in the tree now declares sRGB, and no Display P3 asset exists, so none is offered for visionOS. |
| 8 | The safe field and any margin figure are declared as **this kit's own rule**, never attributed to Apple | Read the guidebook text: the attribution must be explicit | **Met** — The guidebook states the 90-unit field is this kit's own rule and that Apple publishes no numeric safe zone. Clear space is stated as the mark's own proportion. |
| 9 | The type scale has a documented **floor per platform**: 11 pt iOS/iPadOS, 10 pt macOS, 23 pt tvOS, 12 pt visionOS, 12 pt watchOS | Compute the smallest specified size for each surface and compare against the list | **Met** — The book now carries a five-platform floor table with Apple's default and minimum for each, sourced to the Typography page and its 16 December 2025 change log, and states this kit's own web floor against it. |
| 10 | **No Ultralight, Thin or Light weight** appears in any specified role | Search the type tokens for those weight names and for any variable-font weight value below the documented lower bound | **Met** — No weight token exists, so no weight name appears in any role. Shipped CSS uses 400 to 700 only, and both variable fonts have the wght axis clamped at a 400 minimum. |
| 11 | Documentation does **not** claim Dynamic Type support on macOS | Search the guidebook for "Dynamic Type" and check every macOS context | **Met** — The guidebook, its print build and the 77-page PDF contain no occurrence of "Dynamic Type". The kit's one mention states that macOS does not support it and iOS does. |
| 12 | The typeface count is stated, with a reason, and any small-size compensation rules (tracking, width) are written as manual instructions with the sizes at which they apply | Count families in the type tokens; confirm each rule names a size threshold | **Part met** — Two fontFamily tokens, count stated as "Two faces" with a reason per face. NO SIZE-DEPENDENT COMPENSATION RULE EXISTS ANY MORE. This scored Met on the strength of a 12 px Bangla floor and a weight step below 14 px, and both left with the Bangla on 27 August 2026.

The criterion's second clause is conditional — "ANY small-size compensation rules ... are written as manual instructions with the sizes at which they apply" — so with none it is satisfied only vacuously, and this score says so rather than claiming a rule that no longer exists. Literata's opsz axis runs 7 to 72 and redraws the letterform as the size falls, which is the mechanism Noto Serif Bengali lacked and the reason no English floor is warranted: the thin strokes thicken on their own. That is why the honest score is Part met and not Met. |
| 13 | **Every** foreground/background pair is measured and the measured figure recorded — 4.5:1 for text at 17 pt or below at any weight, 3:1 at 18 pt or bold | Run the contrast measurement over the full pair matrix and publish the resulting table, not a sample | **Met** — All 284 recorded pairs recomputed from the hexes with no mismatch and none below target. The book publishes 280 matrix cells plus four role tables. 46380 rendered text nodes held to 4.5. |
| 14 | Non-text contrast reaches **3:1** for interface components and meaningful graphics (WCAG 1.4.11 AA) | Measure borders, icons, focus rings and chart strokes against their adjacent colours | **Met** — 360 focus rings measured at 3:1 from pixels, 3080 icon and graphic readings worst 5.45:1, 21516 border readings worst 4.03:1 against an adjacent colour. No chart component ships. |
| 15 | **Nothing is conveyed by colour alone** | Render every status and semantic state in greyscale; each must remain distinguishable by shape, icon or text | **Met** — 255 status elements, every one carrying words and 220 a distinct glyph. The 15 invalid inputs add a 2 px border plus a linked error line. Greyscale renders stay readable. |
| 16 | Interactive targets meet **24×24 CSS px** (WCAG 2.2 AA), and additionally **44×44 pt** where an Apple platform is specified and **48×48 dp** where Android is | Measure rendered hit areas per platform specification, not drawn box sizes | **Met** — 726 targets measured plus the kit harness's 7592 across four themes: every non-inline control is 24 CSS px or more. No Apple or Android build ships, so those clauses have nothing to bind to. |
| 17 | Spacing between adjacent controls is specified, not only their sizes — approximately 12 pt around bezelled and 24 pt around unbezelled elements where Apple platforms are targeted | Read the component specifications: each must state an inter-control gap | **Met** — The component gap is now a stated rule: adjacent controls at least --as-space-2 which is 12 px, and --as-space-4 which is 24 px where controls have no visible edge. Apple's figures are cited. |
| 18 | The **focus indicator is specified numerically by this kit**, meeting WCAG 2.4.13 geometry (area at least that of a 2 CSS px perimeter) at 3:1 contrast, and is never entirely obscured (2.4.11) | Measure the rendered indicator; attempt to overlap it with sticky page furniture and confirm part remains visible | **Met** — 3px outline at 2px offset from kit tokens; ring area 1224 against the 776 needed; worst contrast 3.8134:1; 726 of 726 rings visible on focus, though a post-focus scroll can hide one behind the sticky bar. |
| 19 | Every motion duration and easing curve is labelled as **this kit's own**, with no attribution to Apple | Read the motion documentation for attribution claims | **Met** — The guidebook states Apple publishes no durations and that every duration and curve is the kit's own. No duration or curve is attributed to Apple in any shipped motion document. |
| 20 | Reduced motion substitutes a **fade**, never a spatial move, depth transition or blur | Enable the reduced-motion setting and observe every animation in the kit | **Met** — Nothing moves, blurs or changes depth, reduced or not — and a fade is now substituted rather than the transition removed. Under reduce the movement duration collapses to 1 ms and the colour duration is held at its full measured value, emitted from duration.motion.colour rather than typed.

Measured, not asserted. 00_sandbox/measure.py reads a movement probe and a colour probe in a real browser under prefers-reduced-motion and reports "movement 0.22s -> 0.001s, cross-fade held at 0.12s"; it also fails if either custom property is UNSET, and checks the value differs with the preference off, so the media query cannot pass by doing nothing. 11_site/check.py does the same on the shipped page. |
| 21 | **No deprecated Material colour role name** (`background`, `onBackground`, `surfaceVariant`) is used as a token name, and any Material mapping counts tonal surfaces correctly — five containers, seven tonal surfaces | Search the token file for the three names; count surface tokens and compare against the written claim | **Met** — No token or CSS variable carries the three deprecated names. Seven surface tokens per theme, and engine.py line 102 states seven tonal surfaces of which five are containers. |
| 22 | The dynamic-colour position is stated explicitly, naming the mechanism — static brand colours, or harmonised via `HarmonizedColors` | Read the guidebook: the decision and the mechanism must both appear | **Met** — The book now states the position and names the mechanism: brand colours stay static, HarmonizedColors is not used, and the opt-in call is named. The cost on Android is stated too. |
| 23 | Any dependency on Material Expressive records the **library version and its channel** (stable 1.4.0 versus 1.5.0-alpha) | Read the dependency manifest: version and channel must both be present | **Met** — Met — and there IS a Material dependency now, where there was none. 15_native/android/build.gradle.kts declares androidx.compose:compose-bom 2026.08.00 with material3, ui and foundation from it, and the channel is recorded in the file: STABLE, not the 1.5.0 alpha where much of Expressive lives. Kotlin 2.4.10 and AGP are pinned beside it. The framework-free token layer still imports no androidx; the Compose layer always did, and it is now compiled against the real library rather than a declared surface. |
| 24 | Tokens validate as **DTCG 2025.10**: only the 13 permitted `$type` values; colour values as structured objects with `colorSpace` and `components`; dimension and duration carrying units even at zero; every alias resolvable | Run schema validation against the format, then resolve every alias and confirm none dangles | **Met** — build.py --check reports 0 problems, and an independent validator over all 25 token documents and 910 tokens found no type, colour, unit or alias fault. |
| 25 | **No pipeline step depends on an archived or expiring dependency** — Material Theme Builder's repository, Figma Code Connect framework-specific parsers after 17 August 2026, the Enterprise-only Figma Variables REST API, or authoring `.fig` files | Read every build script and list each external dependency with its maintenance status and licence | **Met** — Met. The criterion asks about ARCHIVED OR EXPIRING dependencies, and none of the four named traps appears: every upstream repository checked is unarchived with recent commits, and the new Gradle gate resolves androidx.compose from the STABLE channel — 1.4.0 by way of compose-bom 2026.08.00 — rather than the 1.5.0 alpha, which is precisely the kind of pre-release channel this criterion exists to keep out of a pipeline.

One thing DID change on 28 August 2026 and is recorded rather than left for a reader to notice: this verdict used to add "and no build script touches the network". That is no longer true — the Gradle gate resolves from google() and mavenCentral(). It was volunteered evidence rather than what the criterion tests, so it does not move the score, and pretending otherwise would be scoring down for a question nobody asked. The bounds on it are in 15_native/LIMITS.md, including the one that is not yet in place: gradle/verification-metadata.xml, a sha256 per artefact, which needs a JDK no machine here has. |
| 26 | Licences are declared per artefact and are machine-readable — Apache-2.0 for code and tokens, the font's own licence for any bundled font (with Reserved Font Name checked before any fork is renamed), PolyForm Noncommercial 1.0.0 for identity assets using the **no-trailing-slash** canonical URL | Confirm a licence file in each artefact directory, a licence manifest listing SPDX identifiers, and a link check that every licence URL resolves | **Part met** — All 5 licence URLs return 200 and every font ships its OFL. But six artefact directories hold no licence file, spdx.py --check exits 1 on 47 files, and identity is unlicensed rather than PolyForm. |
| 27 | The **brand book and the design system ship as separate artefacts**, with separate licences and separate front doors, and the design system remains usable by someone not permitted to use the mark | Remove the identity assets and confirm the token set still builds and the component documentation still makes sense | **Part met** — Tokens and both packages still verify with 04_mark and 10_assets deleted, and packages ship no identity file. But 08_components and 11_site builds fail-close on the missing mark manifests. |
| 28 | **Every factual claim in the guidebook carries a source and a date checked**, and no number appears that cannot be traced to one | Read every claim: each must have a citation with a URL and a date; count the untraceable numbers, which must be zero | **Part met** — The apparatus now exists and is reachable: 38 distinct real URLs against 2 before, a generated 57-source appendix, and BENCHMARK.md embedded in the book. Per-sentence citation is not met — 68 of 76 external-authority sentences still carry no URL of their own. |

The 4 rows that are not met in full are carried into `01_research/OPEN-FINDINGS.md`, so they sit in the same register as every other open item rather than only here.
