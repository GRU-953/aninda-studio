...
## Clear space — two statements in the source disagree

There is a known contradiction in the system's own files as at 14 August 2026:

- `04_mark/manifest.json`, generated by `04_mark/build.py`, says clear space is
  **half the mark's own height on all four sides**.
- The marks card in `08_components/`, generated by `08_components/build.py`, says
  **one stroke width**.

`scripts/asset.py` uses the manifest — half the mark's height — because the
manifest is the output of the builder that owns the mark. **This needs the
owner's decision, and the losing statement needs correcting at its source.** Do
not quietly follow one and let the other stand.
