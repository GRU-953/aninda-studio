...
"""
    ...
    # Apache-2.0 section 4(a) requires giving every recipient "a copy of this
    # License". Both packages used to ship the APPENDIX boilerplate instead — the
    # 751-byte "How to apply the Apache License to your work" note, which is
    # instructions for an author, not terms for a recipient. Every other bundle in
    # this tree ships the full 11,358-byte text; these two now do as well, read from
    # the repository's own LICENSE rather than retyped, so there is one copy to keep
    # right. They also live in `files`, so --check compares them — they were written
    # outside it before and nothing in CI or verify-all.sh ever looked at them.
    licence_text = (ROOT / "LICENSE").read_text(encoding="utf-8")
    if "TERMS AND CONDITIONS" not in licence_text:
        raise SystemExit(
            "could not run: the repository LICENSE does not hold the Apache-2.0 "
            "terms, so the packages cannot ship a copy of them")
    for sub in ("npm", "python"):
        files[HERE / sub / "LICENSE"] = licence_text
...
def main() -> int:
    ...
    if args.check:
        for path, content in files.items():
            if not path.exists() or path.read_text() != content:
                print(f"\n--check: {path.relative_to(ROOT)} differs from the build",
                      file=sys.stderr)
                return 1
        # And the other direction. Comparing only what the build generates cannot
        # see a file somebody added to a published package — which is the more
        # dangerous drift, because it ships. 13_plugins/claude-design/build.py has
        # swept for extras from the start; these two did not.
        expected = set(files)
        # Anything git ignores is not part of the package as committed — a
        # __pycache__ written by importing the module locally is the obvious case,
        ...
        for sub in ("npm", "python"):
            ...
            for found in sorted(root.rglob("*")):
                if found.resolve() in ignored:
                    continue
                if found.is_file() and found not in expected:
                    print(f"\n--check: {found.relative_to(ROOT)} is in the package "
                          f"and is not generated by this build",
                          file=sys.stderr)
                    return 1
        print(f"\n--check: both packages match the source, {len(files)} files, "
              f"and hold nothing this build did not write. Nothing written.")
        return 0
    ...
    print(f"\nWrote {len(files)} files to {HERE.relative_to(ROOT)}/")
