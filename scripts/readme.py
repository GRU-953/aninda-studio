...
| `15_native/` | Swift and Kotlin — the tokens, {f['components']} SwiftUI components, a Compose theme, and the {f['patterns']} patterns as screens on both, {f['native']} files in all, compiled by the build that gates them |
...
def main() -> int:
    ...
    if f["pub_unpublished"] != 2:
        ...
        return 1
    files = {ROOT / "README.md": english(f)}
